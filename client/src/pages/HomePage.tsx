...
// Classify steps into the three culinary phases
function classifyCookPhase(task: MasterTask): "mise" | "alaminate" | "finale" {
  const lower = task.instruction.toLowerCase();
  // Mise en Place: prep work before heat
  if (/chop|dice|mince|slice|peel|grate|shred|measure|crush|squeeze|mash|mix|whisk|combine|prep|marinate|season.*before|toss.*before|cut|trim/i.test(lower)) return "mise";
  // Finale: plating, resting, serving
  if (/rest|let stand|serve|plate|garnish|slice.*serve|enjoy|transfer.*plate/i.test(lower)) return "finale";
  // A La Minute: active cooking
  return "alaminate";
}

const COOK_PHASE_CONFIG = {
  mise: {
    title: "Mise en Place",
    subtitle: "Prepare & measure everything first",
    emoji: "🔪",
  ...
  alaminate: {
    title: "À La Minute",
    subtitle: "In the moment — the heat is on",
    emoji: "🔥",
  ...
  finale: {
    title: "Mise à Table",
    subtitle: "Rest, plate, and bring it to the table",
    emoji: "🍽️",
...
type CookPhase = "prep" | "map" | "execution" | "done";
...
function StepCook({
  ...
  const [phase, setPhase] = useState<CookPhase>("prep");
  ...
  // All unique ingredients across selected recipes
  const allIngredients = selectedRecipes.flatMap(r => {
    ...
    return r.ingredients.map(ing => ({
      ...
      recipeName: r.name,
      owned: lowerPantry.some(p => ing.name.toLowerCase().includes(p) || p.includes(ing.name.toLowerCase().split(" ")[0])),
    }));
  });
  const toBuy = allIngredients.filter(i => !i.owned);
  const alreadyHave = allIngredients.filter(i => i.owned);

  // Group plan by culinary phase
  const grouped = useMemo(() => {
    const phases: Record<string, MasterTask[]> = { mise: [], alaminate: [], finale: [] };
    plan.forEach(t => { const p = classifyCookPhase(t); phases[p].push(t); });
    return phases;
  }, [plan]);

  // ── PAGE 1: Prep & Shopping ──
  if (phase === "prep") {
    return (
      <div className="max-w-3xl mx-auto px-4 py-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h2 className="text-xl font-display font-bold">Before You Start</h2>
            <p className="text-xs text-muted-foreground">Get your kitchen ready, then review the cook map</p>
          </div>
          <div className="ml-auto flex gap-2">
            <span className="text-xs text-muted-foreground bg-muted px-2 py-1 rounded-full">🔪 Mise en Place</span>
            <span className="text-xs text-muted-foreground bg-muted px-2 py-1 rounded-full hidden sm:block">Page 1 of 2</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          ...
          <div className="rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 p-3 text-center">
            <div className="text-xl font-bold font-display text-emerald-600 dark:text-emerald-400">
              {timeSaved > 0 ? `-${fmtMins(timeSaved)}` : "✔ Optimal"}
            </div>
            <div className="text-xs text-emerald-600/70 dark:text-emerald-400/70">Time saved</div>
          </div>
        </div>

        {/* Mise en Place prep checklist */}
        <div className="rounded-2xl border border-amber-200 dark:border-amber-800 overflow-hidden mb-5">
          <div className="bg-amber-100 dark:bg-amber-900/40 px-4 py-3 flex items-center gap-2">
            <span className="text-lg">🔪</span>
            <div>
              <h3 className="font-display font-bold text-sm">Mise en Place</h3>
              <p className="text-xs text-muted-foreground">Complete all prep before turning on any heat</p>
            </div>
          </div>
          <div className="divide-y divide-border">
            {grouped.mise.length > 0 ? grouped.mise.map((task, i) => (
              <div key={i} className="flex items-start gap-3 px-4 py-3 bg-card hover:bg-amber-50/50 dark:hover:bg-amber-950/20 transition-colors">
                <div className="w-5 h-5 rounded border-2 border-amber-300 dark:border-amber-700 shrink-0 mt-0.5" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm">{task.instruction}</p>
                  <span className="text-[11px] text-muted-foreground">{task.recipeName} · {fmtMins(task.durationMinutes)}</span>
                </div>
              </div>
            )) : (
              <div className="px-4 py-3 text-sm text-muted-foreground bg-card">No dedicated prep steps — these recipes start with heat.</div>
            )}
          </div>
        </div>
        ...
        {toBuy.length > 0 && (
          <div className="rounded-2xl border border-border overflow-hidden mb-6">
            <div className="bg-muted px-4 py-3 flex items-center gap-2">
              ...
              <div>
                <h3 className="font-display font-bold text-sm">Still need to buy ({toBuy.length} items)</h3>
                <p className="text-xs text-muted-foreground">Pick these up before you cook</p>
              </div>
            </div>
            <div className="divide-y divide-border">
              {toBuy.map((ing, i) => (
                <div key={i} className="flex items-center justify-between px-4 py-2.5 bg-card hover:bg-accent/30 transition-colors">
                  <div>
            ...
            {alreadyHave.length > 0 && (
              <div className="border-t border-border bg-muted/30 px-4 py-2.5">
                <p className="text-xs text-muted-foreground font-medium mb-1.5">✓ Already in your pantry</p>
                <div className="flex flex-wrap gap-1.5">
                  {alreadyHave.map((ing, i) => (
                    <span key={i} className="text-[11px] text-muted-foreground/60 line-through">{ing.name}</span>
                  ))}
                </div>
              </div>
            )}
        ...
        <Button size="lg" onClick={() => setPhase("map")}
          className="w-full gap-3 bg-primary text-primary-foreground font-bold py-5 rounded-2xl text-base" data-testid="btn-to-map">
          View Cook Map — Step by Step <ChevronRight size={20} />
        </Button>
      </div>
    );
  }

  // ── PAGE 2: Cook Map (Parallel Flowchart) ──
  if (phase === "map") {
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-6">
        <div className="flex items-center gap-3 mb-5">
          <button onClick={() => setPhase("prep")} className="p-2 rounded-lg hover:bg-accent text-muted-foreground"><ChevronLeft size={18} /></button>
          <div>
          ...
          <span className="ml-auto text-xs text-muted-foreground bg-muted px-2 py-1 rounded-full hidden sm:block">Page 2 of 2</span>
        </div>

        {/* Recipe color key */}
        <div className="flex flex-wrap gap-2 mb-5 p-3 rounded-xl bg-muted/40">
          {selectedRecipes.map((r, i) => (
            <span key={r.id} className="flex items-center gap-1.5 text-xs font-medium">
              <span className={`w-2.5 h-2.5 rounded-full ${RECIPE_PALETTE[i % RECIPE_PALETTE.length].dot}`} />
              {r.name}
            </span>
          ))}
        ...
        {/* ── Flowchart ── */}
        {(["mise", "alaminate", "finale"] as const).map((phaseKey) => {
          const cfg = COOK_PHASE_CONFIG[phaseKey];
          const tasks = grouped[phaseKey];
          if (tasks.length === 0) return null;

          // Group tasks by startMinute so parallel ones are side-by-side
          const timeGroups: Map<number, MasterTask[]> = new Map();
          tasks.forEach(t => {
            const key = t.startMinute;
            if (!timeGroups.has(key)) timeGroups.set(key, []);
            timeGroups.get(key)!.push(t);
          });
          const sortedGroups = Array.from(timeGroups.entries()).sort((a, b) => a[0] - b[0]);

          return (
            <div key={phaseKey} className={`rounded-2xl border-2 ${cfg.border} overflow-hidden mb-4`}>
              {/* Section header */}
              <div className={`${cfg.header} px-4 py-3 flex items-center gap-3`}>
                <span className="text-xl">{cfg.emoji}</span>
                <div>
                  <h3 className={`font-display font-bold text-sm ${cfg.text}`}>{cfg.title}</h3>
                  <p className="text-xs text-muted-foreground">{cfg.subtitle}</p>
                </div>
                <span className="ml-auto text-xs font-mono text-muted-foreground">{tasks.length} step{tasks.length !== 1 ? "s" : ""}</span>
              </div>

              {/* Parallel rows */}
              <div className={`${cfg.bg} p-3 space-y-2`}>
                {sortedGroups.map(([minute, groupTasks], gi) => (
                  <div key={gi}>
                    {/* Time marker */}
                    {groupTasks.length > 1 && (
                      <div className="flex items-center gap-2 mb-1.5">
                        <div className="h-px flex-1 bg-border/60" />
                        <span className="text-[10px] text-muted-foreground font-mono px-1">{minute > 0 ? `+${minute}m` : "Start"}</span>
                        <div className="h-px flex-1 bg-border/60" />
                        <span className="text-[10px] font-semibold text-muted-foreground">⇆ parallel</span>
                        <div className="h-px flex-1 bg-border/60" />
                      </div>
                    )}
                    {/* Blocks: side-by-side if parallel */}
                    <div className={`grid gap-2 ${groupTasks.length > 1 ? "grid-cols-2" : "grid-cols-1"}`}>
                      {groupTasks.map((task, ti) => {
                        const recipeIdx = selectedRecipes.findIndex(r => r.id === task.recipeId);
                        const pal = task.type === "consolidated" ? null : RECIPE_PALETTE[recipeIdx % RECIPE_PALETTE.length];
                        const isConsolidated = task.type === "consolidated";
                        ...
                        return (
                          <div key={ti}
                            ...
                            data-testid={`map-task-${gi}-${ti}`}
                          >
                            {/* Recipe tag */}
                            <div className="flex items-center gap-1.5 mb-1.5">
                              <span className={`w-2 h-2 rounded-full ${isConsolidated ? "bg-violet-400" : (pal?.dot ?? "bg-muted-foreground")}`} />
                              <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide">
                                {isConsolidated ? "All recipes" : task.recipeName}
                              </span>
                              <span className="ml-auto text-[10px] font-mono text-muted-foreground">{fmtMins(task.durationMinutes)}</span>
                            </div>
                            <p className="text-sm leading-snug font-medium">{task.instruction}</p>
                            {task.equipment && (
                              <p className="text-[11px] text-muted-foreground mt-1">{EQUIPMENT_INFO[task.equipment as EquipmentKey]?.icon ?? ""} {task.equipment}</p>
                            )}
                          </div>
                        );
                      })}
              ...
              {phaseKey !== "finale" && (
                <div className="flex items-center justify-center py-3 border-t border-dashed border-border bg-background">
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    <div className="h-px w-8 bg-border" />
                    <span>{phaseKey === "mise" ? "🔥 Heat goes on" : "🍽️ Almost ready"}</span>
                    <div className="h-px w-8 bg-border" />
                  </div>
                </div>
              )}
        ...
        <Button size="lg" onClick={startCooking}
          className="w-full gap-3 bg-primary text-primary-foreground font-bold py-5 rounded-2xl text-base sticky bottom-4 mt-4" data-testid="btn-start-cooking">
  ...
  if (phase === "execution") {
    ...
    const prev = stepIdx > 0 ? plan[stepIdx - 1] : null;
    const isLast = stepIdx === plan.length - 1;
    const isFirst = stepIdx === 0;
    const progress = Math.round(((stepIdx + 1) / plan.length) * 100);
    const cookPhase = classifyCookPhase(current);
    const phaseCfg = COOK_PHASE_CONFIG[cookPhase];

    return (
      <div className="min-h-[calc(100vh-120px)] flex flex-col" data-testid="execution-mode">
        {/* Progress bar */}
        <div className="h-1.5 bg-muted"><div className="h-full bg-primary transition-all duration-500" style={{ width: `${progress}%` }} /></div>
        ...
        <div className="flex items-center justify-between px-4 py-2 bg-muted/40 border-b border-border">
          ...
          <div className="flex items-center gap-2 text-xs">
            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${phaseCfg.header} ${phaseCfg.text}`}>
              {phaseCfg.emoji} {phaseCfg.title}
            </span>
          </div>
        ...
        <div className="flex-1 flex flex-col justify-center px-4 py-6 max-w-2xl mx-auto w-full">
          ...
          <div className="flex items-center justify-center gap-2 mb-4">
            <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
              {current.type === "consolidated" ? "All recipes" : current.recipeName}
            </span>
            {current.equipment && (
              <span className="text-xs text-muted-foreground">· {current.equipment}</span>
            )}
          </div>

          {/* ACTIVE STEP — clean, no color fill */}
          <div className="rounded-3xl border-2 border-border bg-card p-8 mb-5 text-center shadow-sm" data-testid="active-step">
            <div className="flex items-center justify-center gap-1.5 text-sm font-medium mb-4 text-muted-foreground">
              <Clock size={14} />
          ...
          {next && (
            <div className="rounded-2xl border border-dashed border-border bg-muted/30 px-5 py-4 mb-6" data-testid="next-step-preview">
              ...
              <p className="text-[11px] text-muted-foreground/60 mt-1">
                {next.type === "consolidated" ? "All recipes" : next.recipeName} · {fmtMins(next.durationMinutes)}
              </p>
            </div>
          )}
          ...
          <div className="flex items-center justify-center gap-1 mb-6 flex-wrap">
            {plan.map((_, i) => (
              <button key={i} onClick={() => setStepIdx(i)}
                className={`rounded-full transition-all ${i < stepIdx ? "w-2 h-2 bg-primary/40 hover:bg-primary/70" : i === stepIdx ? "w-3 h-3 bg-primary" : "w-2 h-2 bg-muted-foreground/20 hover:bg-muted-foreground/40"}`}
          ...
          <div className="flex gap-3">
            {!isFirst && (
              <Button variant="outline" onClick={() => setStepIdx(i => i - 1)}
                className="gap-1 shrink-0" data-testid="btn-prev-step">
                <ChevronLeft size={18} /> Back
              </Button>
            )}
            {isLast ? (
              <Button size="lg" onClick={() => { setRunning(false); setPhase("done"); }}
                className="flex-1 gap-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-5 rounded-2xl text-base" data-testid="btn-finish">
                <CheckCircle2 size={22} /> Done — Let's eat!
              </Button>
            ) : (
              <Button size="lg" onClick={() => setStepIdx(i => i + 1)}
                className="flex-1 gap-3 bg-primary text-primary-foreground font-bold py-5 rounded-2xl text-base" data-testid="btn-next-step">
